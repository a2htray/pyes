...
def _es_comma(n_dimension, objective, bounds, n_iter, learn_rate, mu, lam, random_state=None, event_on_generation=EVENT_ON_GENERATION):
    ...
    for epoch in range(n_iter):
        ...
        event_on_generation(epoch, asarray(population))
...
# evolution strategy (mu + lambda) algorithm
def _es_plus(n_dimension, objective, bounds, n_iter, learn_rate, mu, lam, random_state=None, event_on_generation=EVENT_ON_GENERATION):
    rs = random.RandomState(random_state)
    best, best_eval = None, 1e+10
    n_children = int(lam / mu)
...
def ES(comma_or_plus: str, n_dimension: int, objective, bounds, n_iter, learn_rate, mu, lam, random_state=None, event_on_generation=EVENT_ON_GENERATION):
